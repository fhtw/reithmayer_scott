...
public class NavPlugin extends PluginClass {
    ...
    public void start(Socket socket, UrlClass url) {
        ArrayList<String> urlParts = url.getParameters();
        int count = urlParts.size();

        if (count < 2) { //Anzahl der Parameter
            this.search();
        } else if (urlParts.get(0).equalsIgnoreCase("s")) {
            this.result(url);
        } else {
            this.load();
            this.search();
    ...
    private void result(UrlClass url) {
        String street = url.getParameters().get(1);
        ...
        try (FileWriter writer = new FileWriter(f)) {
            ...
            if (!tags.containsKey(street)) {
                ...
                for (Object citie : cities) {
                    out.write("<p>");
                    out.write(citie.toString());
                    out.write("</p>\n");
                }
            }
